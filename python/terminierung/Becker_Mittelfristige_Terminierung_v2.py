...
def optimize_sequence_ga(
    ...
    for g in range(generations):
        ...
        mu_var_pairs: List[Tuple[float, float]] = []
        for seq in pop:
            key = tuple(seq)
            if key in cache:
                mu, var = cache[key]
        ...
        if gen_best_val < best_val:
            ...
            best_components = mu_var_pairs[gen_best_idx]
...
def compute_plan_metrics(plan: Sequence[Dict[str, float]]) -> Dict[str, float]:
    if not plan:
        return {"meanProcTime": 0.0, "totalTardiness": 0.0, "maxTardiness": 0.0}
...
def prepare_debug_entries(
    ...
    debug.extend(create_plot_entries(history, baseline_plan_metrics, optimized_plan_metrics))
    return debug
...
def schedule_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    mutation_rate = max(0.0, min(1.0, float(ga_config.get("mutationRate", 0.25) or 0.25)))
    ...
    baseline_mu, baseline_var = simulate_sequence(baseline_seq, ga_orders, reps, seed * 19 + 7)
